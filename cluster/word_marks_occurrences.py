...
df_films = spark.read.parquet("films.parquet")
df_words = df_films.select('*', psf.explode('subtitles').alias('words'))
df_words = df_words.select('*', psf.explode('words').alias('word'))

df_word_occurrences_films = df_words.groupBy("tconst", "word").count()
df_word_occurrences_total = df_words.groupBy("word").count()

df_word_occurrences_films.write.parquet("words/filmwords.parquet")
df_word_occurrences_total.write.parquet("words/allwords.parquet")